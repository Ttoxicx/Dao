...
layout(set=0,binding=0) uniform unused_name_perframe{
...
void main(){
	...
	float projectvel_x=dot(vel_dir,right_direction);
	...
	if(abs(projectvel_x)<size_x||abs(projectvel_y)<size_y){
		...
		vec3 project_dir=normalize(projectvel_x*right_direction+projectvel_y*up_direction);
		vec3 size_dir=normalize(cross(forward_direction,project_dir));
		world_position=size_x*size_dir*model_position.x+size_y*project_dir*model_position.y+anchor_location;
	}
