...
namespace Dao {
	void MeshComponent::postLoadResource(std::weak_ptr<GObject> parent_object) {
		...
		_raw_meshes.resize(_mesh_res.m_sub_meshes.size());
		size_t raw_mesh_count = 0;
		for (const SubMeshRes& sub_mesh : _mesh_res.m_sub_meshes) {
			GameObjectPartDesc& mesh_part = _raw_meshes[raw_mesh_count];
			mesh_part.m_mesh_desc.m_mesh_file = asset_manager->getFullPath(sub_mesh.m_obj_file_ref).generic_string();
			mesh_part.m_material_desc.m_with_texture = sub_mesh.m_material.empty() == false;

			if (mesh_part.m_material_desc.m_with_texture) {
				MaterialRes material_res;
				asset_manager->loadAsset(sub_mesh.m_material, material_res);
				mesh_part.m_material_desc.m_base_color_texture_file = asset_manager->getFullPath(material_res.m_base_colour_texture_file).generic_string();
				mesh_part.m_material_desc.m_metallic_roughness_texture_file = asset_manager->getFullPath(material_res.m_metallic_roughness_texture_file).generic_string();
				mesh_part.m_material_desc.m_normal_texture_file = asset_manager->getFullPath(material_res.m_normal_texture_file).generic_string();
				mesh_part.m_material_desc.m_occlusion_texture_file = asset_manager->getFullPath(material_res.m_occlusion_texture_file).generic_string();
				mesh_part.m_material_desc.m_emissive_texture_file = asset_manager->getFullPath(material_res.m_emissive_texture_file).generic_string();
			}

			auto object_space_transform = sub_mesh.m_transform.getMatrix();
			mesh_part.m_transform_desc.m_transform_matrix = object_space_transform;
			++raw_mesh_count;
		}
	}
	...
	void MeshComponent::tick(float delta_time) {
		...
		if (transform_component->isDirty()) {
			...
			for (auto& mesh_part : _raw_meshes) {
