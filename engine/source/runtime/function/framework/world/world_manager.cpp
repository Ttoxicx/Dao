...
namespace Dao {
	...
	void WorldManager::initialize() {
		...
		_current_world_url = g_runtime_global_context.m_config_manager->getDefaultWorldUrl();
	}
	...
	void WorldManager::clear() {
		...
		_current_world_resource.reset();
		_current_world_url.clear();
		_is_world_loaded = false;
	}

	void WorldManager::tick(float delta_time) {
		if (!_is_world_loaded) {
			loadWorld(_current_world_url);
		}
	...
	bool WorldManager::loadWorld(const std::string& world_url) {
		...
		_current_world_resource = std::make_shared<WorldRes>(world_res);
		...
		auto itr = m_loaded_levels.find(world_res.m_default_level_url);
		ASSERT(itr != m_loaded_levels.end());

		m_current_active_level = itr->second;
		_is_world_loaded = true;

		LOG_INFO("world loaded succeed!");
		return true;
	}
	...
	void WorldManager::reloadCurrentLevel() {
		...
		auto itr = m_loaded_levels.find(level_url);
		ASSERT(itr != m_loaded_levels.end());
		m_current_active_level = itr->second;
