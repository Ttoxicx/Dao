...
namespace Dao {
	void ParticleManager::initialize() {
		...
		if (global_particle_res.m_emit_gap % 2) {
			...
			_global_particle_res.m_emit_gap = s_default_particle_emit_gap;
		}

		if (global_particle_res.m_time_step < 1e-6) {
			LOG_ERROR("time_step should be larger");
			_global_particle_res.m_time_step = s_default_particle_time_step;
		}

		if (global_particle_res.m_max_life < _global_particle_res.m_time_step) {
			LOG_ERROR("max_life should be larger");
			_global_particle_res.m_max_life = s_default_particle_life_time * s_default_particle_time_step;// ??? 
		}

		_global_particle_res = global_particle_res;
	}

	void ParticleManager::createParticleEmitter(
		const ParticleComponentRes& particle_res,
		...
		transform_desc.m_id = ParticleEmitterIDAllocator::allocate();
	}

	const GlobalParticleRes& ParticleManager::getGlobalParticleRes() {
		return _global_particle_res;
	}
}
