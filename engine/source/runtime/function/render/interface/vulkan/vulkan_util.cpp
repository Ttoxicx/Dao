...
namespace Dao {
	std::unordered_map<uint32_t, VkSampler> VulkanUtil::_mipmap_sampler_map;
	VkSampler								VulkanUtil::_nearset_sampler = VK_NULL_HANDLE;
	VkSampler								VulkanUtil::_linear_sampler = VK_NULL_HANDLE;
	...
	VkShaderModule VulkanUtil::createShaderModule(
		VkDevice device,
		...
		if (vkCreateShaderModule(device, &shader_module_create_info, nullptr, &shader_module) != VK_SUCCESS) {
			LOG_ERROR("shader create failed");
			return VK_NULL_HANDLE;
		}
		return shader_module;
	}

	void VulkanUtil::createBufferAndInitalize(
		VkDevice device,
		VkPhysicalDevice physical_device,
		...
		bufferCreateInfo.usage = buffer_usage_flags;
		...
		for (uint32_t i = 0; i < deviceMemoryProperties.memoryTypeCount; ++i) {
			if (memoryReqs.memoryTypeBits & 1) {
				if ((deviceMemoryProperties.memoryTypes[i].propertyFlags & memory_property_flags) == memory_property_flags) {
	...
	void VulkanUtil::createBuffer(
		...
		allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
		allocInfo.allocationSize = memoryReqs.size;
		allocInfo.memoryTypeIndex = VulkanUtil::findMemoryType(physical_device, memoryReqs.memoryTypeBits, properties);
		...
		if (vkBindBufferMemory(device, buffer, buffer_memory, 0) != VK_SUCCESS) {
			LOG_ERROR("vkBindBufferMemory failed!");
			return;
		}
	}
	...
	void VulkanUtil::copyBuffer(
		...
		VkCommandBuffer commandBuffer = ((VulkanCommandBuffer*)rhi_command_buffer)->getResource();
	...
	void VulkanUtil::createImage(
		...
		allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
		allocInfo.allocationSize = memoryReqs.size;
		allocInfo.memoryTypeIndex = VulkanUtil::findMemoryType(physical_device, memoryReqs.memoryTypeBits, memory_property_flags);
		if (vkAllocateMemory(device, &allocInfo, nullptr, &memory) != VK_SUCCESS) {
			LOG_ERROR("failed to allocate image memory!");
			return;
		}
		if (vkBindImageMemory(device, image, memory, 0) != VK_SUCCESS) {
			LOG_ERROR("vkBindImageMemory failed!");
		}
	}
	...
	VkImageView VulkanUtil::createImageView(
		...
		if (vkCreateImageView(device, &image_view_create_info, nullptr, &image_view) != VK_SUCCESS) {
			LOG_FATAL("vkCreateImageView failed");
		}
		return image_view;
	}
	...
	void VulkanUtil::createGlobalImage(
		...
		case RHIFormat::RHI_FORMAT_R32G32B32A32_SFLOAT:
			...
			break;
		default:
			LOG_ERROR("invalid texture byte size");
			break;
		}
		...
		vmaCreateImage(
			static_cast<VulkanRHI*>(rhi)->m_assets_allocator,
			&image_create_info,
			&allocInfo,
			&image,
			&image_allocation,
		...
		VulkanUtil::copyBufferToImage(rhi, inefficient_staging_buffer, image, texture_image_width, texture_image_height, 1);
		transitionImageLayout(
			rhi,
			image,
			VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
	...
	void VulkanUtil::generateTextureMipmaps(
		RHI* rhi,
		VkImage image,
		...
		if (!(format_properties.optimalTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT)) {
			LOG_ERROR("linear bliting not supported");
			return;
		}
		RHICommandBuffer* rhi_command_buffer = static_cast<VulkanRHI*>(rhi)->beginSingleTimeCommands();
	...
	void VulkanUtil::transitionImageLayout(
		...
		if (old_layout == VK_IMAGE_LAYOUT_UNDEFINED && 
			...
			LOG_ERROR("unsuppoted layout transition!");
			return;
		}
		vkCmdPipelineBarrier(
			command_buffer, sourceStage, destinationStage, 
			0, 0, nullptr, 0, nullptr, 1, &barrier
		);
		static_cast<VulkanRHI*>(rhi)->endSingleTimeCommands(rhi_command_buffer);
	}

	void VulkanUtil::copyBufferToImage(
		RHI* rhi,
		VkBuffer buffer,
		VkImage image,
		uint32_t width,
		...
		region.imageSubresource.layerCount = layout_count;
	...
	void VulkanUtil::genMipmappedImage(
		RHI* rhi,
		VkImage image,
		uint32_t width,
		uint32_t height,
	...
	VkSampler VulkanUtil::getOrCreateMipmapSampler(
		VkPhysicalDevice physical_device,
		VkDevice device,
		uint32_t width,
		uint32_t height
	...
	VkSampler VulkanUtil::getOrCreateNearestSampler(VkPhysicalDevice physical_device, VkDevice device) {
		if (_nearset_sampler == VK_NULL_HANDLE) {
			VkPhysicalDeviceProperties physical_device_properties{};
			vkGetPhysicalDeviceProperties(physical_device, &physical_device_properties);
			VkSamplerCreateInfo samplerInfo{};
			samplerInfo.sType = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO;
			...
			samplerInfo.borderColor = VK_BORDER_COLOR_INT_OPAQUE_BLACK;
			samplerInfo.unnormalizedCoordinates = VK_FALSE;
			if (vkCreateSampler(device, &samplerInfo, nullptr, &_nearset_sampler) != VK_SUCCESS) {
				LOG_ERROR("vkCreateSampler failed!");
			}
		}
		return _nearset_sampler;
	}

	VkSampler VulkanUtil::getOrCreateLinearSampler(VkPhysicalDevice physical_device, VkDevice device) {
		if (_linear_sampler == VK_NULL_HANDLE) {
			VkPhysicalDeviceProperties physical_device_properties{};
			vkGetPhysicalDeviceProperties(physical_device, &physical_device_properties);
			VkSamplerCreateInfo samplerInfo{};
			samplerInfo.sType = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO;
			...
			samplerInfo.borderColor = VK_BORDER_COLOR_INT_OPAQUE_BLACK;
			samplerInfo.unnormalizedCoordinates = VK_FALSE;
			if (vkCreateSampler(device, &samplerInfo, nullptr, &_linear_sampler) != VK_SUCCESS) {
				LOG_ERROR("vkCreateSampler failed!");
			}
		}
	...
	void VulkanUtil::destroyLinearSampler(VkDevice device) {
		vkDestroySampler(device, _linear_sampler, nullptr);
		_linear_sampler = VK_NULL_HANDLE;
	}
}