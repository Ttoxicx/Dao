...
namespace Dao {
	void PickPass::initialize(const RenderPassInitInfo* init_info) {
		...
		const PickPassInitInfo* info = static_cast<const PickPassInitInfo*>(init_info);
		_per_mesh_layout = info->per_mesh_layout;
	...
	void PickPass::setupAttachments() {
		...
		m_rhi->createImageView(
			...
			1, 1, m_framebuffer.attachments[0].view
		);
	}
	...
	void PickPass::setupRenderPass() {
		...
		RHIAttachmentDescription depth_stencil_attachment_description{};
		depth_stencil_attachment_description.format = m_rhi->getDepthImageInfo().depth_image_format;
		depth_stencil_attachment_description.samples = RHI_SAMPLE_COUNT_1_BIT;
		depth_stencil_attachment_description.loadOp = RHI_ATTACHMENT_LOAD_OP_CLEAR;
		depth_stencil_attachment_description.storeOp = RHI_ATTACHMENT_STORE_OP_DONT_CARE;
		depth_stencil_attachment_description.stencilLoadOp = RHI_ATTACHMENT_LOAD_OP_DONT_CARE;
		depth_stencil_attachment_description.stencilStoreOp = RHI_ATTACHMENT_STORE_OP_DONT_CARE;
		depth_stencil_attachment_description.initialLayout = RHI_IMAGE_LAYOUT_UNDEFINED;
		depth_stencil_attachment_description.finalLayout = RHI_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;

		RHIAttachmentDescription attachments[2] = { color_attachment_description,depth_stencil_attachment_description };
		...
		RHIAttachmentReference depth_stencil_attachment_reference{};
		depth_stencil_attachment_reference.attachment = 1;
		depth_stencil_attachment_reference.layout = RHI_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
		...
		if (m_rhi->createRenderPass(&renderpass_create_info, m_framebuffer.render_pass) != RHI_SUCCESS) {
			LOG_FATAL("failed to create inefficient pick render pass");
		}
	}
	...
	void PickPass::setupFramebuffer() {
		...
		if (m_rhi->createFramebuffer(&framebuffer_create_info, m_framebuffer.framebuffer) != RHI_SUCCESS) {
			LOG_FATAL("failed to create inefficient pick framebuffer");
		}
	}
	...
	void PickPass::setupDescriptorSetLayout() {
		...
		RHIDescriptorSetLayoutBinding& mesh_inefficient_pick_global_layout_perdrawcall_vertex_blending_storage_buffer_binding = mesh_inefficient_pick_global_layout_bindings[2];
		...
		mesh_inefficient_pick_global_layout_perdrawcall_vertex_blending_storage_buffer_binding.pImmutableSamplers = nullptr;
		...
		mesh_inefficient_pick_global_layout_create_info.bindingCount = sizeof(mesh_inefficient_pick_global_layout_bindings) / sizeof(mesh_inefficient_pick_global_layout_bindings[0]);
		mesh_inefficient_pick_global_layout_create_info.pBindings = mesh_inefficient_pick_global_layout_bindings;

		if (m_rhi->createDescriptorSetLayout(&mesh_inefficient_pick_global_layout_create_info, m_descriptor_infos[0].layout) != RHI_SUCCESS) {
			LOG_FATAL("failed to create mesh inefficient pick global layout");
		}
	}
	...
	void PickPass::setupPipelines() {
		...
		if (m_rhi->createPipelineLayout(&pipeline_layout_create_info, m_render_pipelines[0].layout) != RHI_SUCCESS) {
			LOG_FATAL("failed to create mesh inefficient pick pipeline layout");
		}
		...
		if (m_rhi->createGraphicsPipelines(RHI_NULL_HANDLE, 1, &pipeline_info, m_render_pipelines[0].pipeline) != RHI_SUCCESS) {
			LOG_FATAL("failed to create mesh inefficient pick graphics pipeline");
		}
	...
	void PickPass::setupDescriptorSet() {
		...
		if (m_rhi->allocateDescriptorSets(&mesh_inefficient_pick_global_descriptor_set_alloc_info, m_descriptor_infos[0].descriptor_set) != RHI_SUCCESS) {
			LOG_FATAL("failed to allocate mesh inefficient pick global descriptor set");
		}
		...
		mesh_inefficient_pick_perframe_storage_buffer_info.buffer = m_global_render_resource->m_storage_buffer.m_global_upload_ringbuffer;

		ASSERT(mesh_inefficient_pick_perframe_storage_buffer_info.range < m_global_render_resource->m_storage_buffer.m_max_storage_buffer_range);
		...
		mesh_inefficient_pick_perdrawcall_storage_buffer_info.buffer = m_global_render_resource->m_storage_buffer.m_global_upload_ringbuffer;

		ASSERT(mesh_inefficient_pick_perdrawcall_storage_buffer_info.range < m_global_render_resource->m_storage_buffer.m_max_storage_buffer_range);
		...
		mesh_inefficient_pick_perdrawcall_vertex_blending_storage_buffer_info.buffer = m_global_render_resource->m_storage_buffer.m_global_upload_ringbuffer;

		ASSERT(mesh_inefficient_pick_perdrawcall_vertex_blending_storage_buffer_info.range < m_global_render_resource->m_storage_buffer.m_max_storage_buffer_range);
	...
	uint32_t PickPass::pick(const Vector2& picked_uv) {
		...
		m_global_render_resource->m_storage_buffer.m_global_upload_ringbuffers_end[m_rhi->getCurrentFrameIndex()] = m_global_render_resource->m_storage_buffer.m_global_upload_ringbuffers_begin[m_rhi->getCurrentFrameIndex()];
		...
		if (m_rhi->beginCommandBufferPFN(m_rhi->getCurrentCommandBuffer(), &command_buffer_begin_info) != RHI_SUCCESS) {
			LOG_FATAL("failed to begin pick pass command buffer");
		}
		...
		m_global_render_resource->m_storage_buffer.m_global_upload_ringbuffers_end[m_rhi->getCurrentFrameIndex()] = perframe_dynamic_offset + sizeof(MeshInefficientPickPerframeStorageBufferObject);
		...
		(*reinterpret_cast<MeshInefficientPickPerframeStorageBufferObject*>(reinterpret_cast<uintptr_t>(m_global_render_resource->m_storage_buffer.m_global_upload_ringbuffer_memory_pointer) + perframe_dynamic_offset)) = _mesh_inefficient_pick_perframe_storage_buffer_object;
		...
		for (auto& pair1 : main_camera_mesh_drawcall_batch) {
			...
			for (auto& pair2 : mesh_instanced) {
				...
				if (total_instance_count > 0) {
					...
					uint32_t drawcall_max_instance_count = (sizeof(MeshInefficientPickPerdrawcallStorageBufferObject::model_matrices) / sizeof(MeshInefficientPickPerdrawcallStorageBufferObject::model_matrices[0]));
					...
					for (uint32_t drawcall_index = 0; drawcall_index < drawcall_count; ++drawcall_index) {
						...
						m_global_render_resource->m_storage_buffer.m_global_upload_ringbuffers_end[m_rhi->getCurrentFrameIndex()] = perdrawcall_dynamic_offset + sizeof(MeshInefficientPickPerdrawcallStorageBufferObject);
						...
						MeshInefficientPickPerdrawcallStorageBufferObject& perdrawcall_storage_buffer_object = (*reinterpret_cast<MeshInefficientPickPerdrawcallStorageBufferObject*>(
						...
						if (mesh.enable_vertex_blending) {
							...
							m_global_render_resource->m_storage_buffer.m_global_upload_ringbuffers_end[m_rhi->getCurrentFrameIndex()] = per_drawcall_vertex_blending_dynamic_offset + sizeof(MeshInefficientPickPerdrawcallVertexBlendingStorageBufferObject);
							...
							MeshInefficientPickPerdrawcallVertexBlendingStorageBufferObject& perdrawcall_vertex_blending_storage_buffer_object = (*reinterpret_cast<MeshInefficientPickPerdrawcallVertexBlendingStorageBufferObject*>(
		...
		if (m_rhi->queueSubmit(m_rhi->getGraphicsQueue(), 1, &submit_info, m_rhi->getFenceList()[m_rhi->getCurrentFrameIndex()]) != RHI_SUCCESS) {
			LOG_FATAL("failed to submit in pick up pass");
		}
		m_rhi->setCurrentFrameIndex((m_rhi->getCurrentFrameIndex() + 1) % m_rhi->getMaxFramesInFlight());
		//implicit host read barrier
		if (m_rhi->waitForFencesPFN(m_rhi->getMaxFramesInFlight(), m_rhi->getFenceList(), RHI_TRUE, UINT64_MAX) != RHI_SUCCESS) {
			LOG_FATAL("failed to wait pick up pass fences");
		}
