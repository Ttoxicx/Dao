...
namespace Dao {
	void ToneMappingPass::initialize(const RenderPassInitInfo* init_info) {
        ...
        const ToneMappingPassInitInfo* info = static_cast<const ToneMappingPassInitInfo*>(init_info);
    ...
    void ToneMappingPass::setupPipelines() {
        ...
        pipeline_Info.subpass = main_camera_subpass_tone_mapping;
    ...
    void ToneMappingPass::updateAfterFramebufferRecreate(RHIImageView* input_attachment) {
        ...
        post_process_perframe_input_attachment_info.sampler = m_rhi->getOrCreateDefaultSampler(Default_Sampler_Nearest);
