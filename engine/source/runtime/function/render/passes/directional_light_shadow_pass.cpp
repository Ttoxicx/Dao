...
namespace Dao {
	...
	void DirectionalLightShadowPass::setupRenderPass() {
		...
		/*
		RHISubpassDependency 结构体的成员定义了这种依赖关系的具体细节：
		1. srcSubpass 和 dstSubpass：
				srcSubpass：源子通道的索引，表示依赖关系的起点。
				dstSubpass：目标子通道的索引，表示依赖关系的终点。如果设置为 RHI_SUBPASS_EXTERNAL，表示依赖于渲染通道外部的操作。
		2. srcStageMask 和 dstStageMask：
				srcStageMask：源子通道需要等待的管线阶段掩码，表示在这个阶段完成的操作。
				dstStageMask：目标子通道需要等待的管线阶段掩码，表示在这个阶段开始的操作。
		3. srcAccessMask 和 dstAccessMask：
				srcAccessMask：源子通道需要等待的访问掩码，表示在这个阶段完成的具体操作（如写入）。
				dstAccessMask：目标子通道需要等待的访问掩码，表示在这个阶段开始的具体操作（如读取）。
		4. dependencyFlags：
				依赖标志，用于指定依赖关系的特殊属性（如按区域依赖）。
		*/
		...
		if (m_rhi->createRenderPass(&renderpass_create_info, m_framebuffer.render_pass) != RHI_SUCCESS) {
			LOG_FATAL("failed to create directional light shadow render pass");
		}
	}
	...
	void DirectionalLightShadowPass::setupFramebuffer() {
		...
		if (m_rhi->createFramebuffer(&framebuffer_create_info, m_framebuffer.framebuffer) != RHI_SUCCESS) {
			LOG_FATAL("failed to create directional light shadow framebuffer");
		}
	}
	...
	void DirectionalLightShadowPass::setupDescriptorSetLayout() {
		...
		RHIDescriptorSetLayoutBinding& mesh_directional_light_shadow_global_layout_perframe_storage_buffer_binding = mesh_directional_light_shadow_global_layout_bindings[0];
		...
		RHIDescriptorSetLayoutBinding& mesh_directional_light_shadow_global_layout_per_drawcall_storage_buffer_binding = mesh_directional_light_shadow_global_layout_bindings[1];
		mesh_directional_light_shadow_global_layout_per_drawcall_storage_buffer_binding.binding = 1;
		mesh_directional_light_shadow_global_layout_per_drawcall_storage_buffer_binding.descriptorType = RHI_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC;
		mesh_directional_light_shadow_global_layout_per_drawcall_storage_buffer_binding.descriptorCount = 1;
		mesh_directional_light_shadow_global_layout_per_drawcall_storage_buffer_binding.stageFlags = RHI_SHADER_STAGE_VERTEX_BIT;

		RHIDescriptorSetLayoutBinding& mesh_directional_light_shadow_global_layout_per_drawcall_vertex_blending_storage_buffer_binding = mesh_directional_light_shadow_global_layout_bindings[2];
		mesh_directional_light_shadow_global_layout_per_drawcall_vertex_blending_storage_buffer_binding.binding = 2;
		mesh_directional_light_shadow_global_layout_per_drawcall_vertex_blending_storage_buffer_binding.descriptorType = RHI_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC;
		...
		RHIDescriptorSetLayoutCreateInfo mesh_directional_light_shadow_global_layout_create_info;
		mesh_directional_light_shadow_global_layout_create_info.sType = RHI_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
		mesh_directional_light_shadow_global_layout_create_info.pNext = nullptr;
		mesh_directional_light_shadow_global_layout_create_info.flags = 0;
		mesh_directional_light_shadow_global_layout_create_info.bindingCount = sizeof(mesh_directional_light_shadow_global_layout_bindings) / sizeof(mesh_directional_light_shadow_global_layout_bindings[0]);
		mesh_directional_light_shadow_global_layout_create_info.pBindings = mesh_directional_light_shadow_global_layout_bindings;

		if (m_rhi->createDescriptorSetLayout(&mesh_directional_light_shadow_global_layout_create_info, m_descriptor_infos[0].layout) != RHI_SUCCESS) {
			LOG_FATAL("failed to create mesh directional light shadow global layout");
		}
	}
	...
	void DirectionalLightShadowPass::setupPipelines() {
		...
		if (m_rhi->createPipelineLayout(&pipeline_layout_create_info, m_render_pipelines[0].layout) != RHI_SUCCESS) {
			LOG_FATAL("failed to create mesh directional light shadow pipeline layout");
		}
		...
		rasterization_state_create_info.depthBiasClamp = RHI_FALSE;
		...
		if (m_rhi->createGraphicsPipelines(RHI_NULL_HANDLE, 1, &pipeline_info, m_render_pipelines[0].pipeline) != RHI_SUCCESS) {
			LOG_FATAL("failed to create mesh directional light shadow graphics pipeline");
		}
	...
	void DirectionalLightShadowPass::setupDescriptorSet() {
		...
		if (m_rhi->allocateDescriptorSets(&mesh_directional_light_shadow_global_descriptor_set_alloc_info, m_descriptor_infos[0].descriptor_set) != RHI_SUCCESS) {
			LOG_FATAL("falied to allocate directional light shadow global descriptor set");
		}
		...
		mesh_directional_light_shadow_perframe_storage_buffer_info.buffer = m_global_render_resource->m_storage_buffer.m_global_upload_ringbuffer;

		ASSERT(mesh_directional_light_shadow_perframe_storage_buffer_info.range < m_global_render_resource->m_storage_buffer.m_max_storage_buffer_range);
		...
		mesh_directional_light_shadow_perdrawcall_storage_buffer_info.buffer = m_global_render_resource->m_storage_buffer.m_global_upload_ringbuffer;

		ASSERT(mesh_directional_light_shadow_perdrawcall_storage_buffer_info.range < m_global_render_resource->m_storage_buffer.m_max_storage_buffer_range);

		RHIDescriptorBufferInfo mesh_directional_light_shadow_perdrawcall_vertex_blending_storage_buffer_info = {};
		mesh_directional_light_shadow_perdrawcall_vertex_blending_storage_buffer_info.offset = 0;
		mesh_directional_light_shadow_perdrawcall_vertex_blending_storage_buffer_info.range = sizeof(MeshDirectionalLightShadowPerDrawcallVertexBlendingStorageBufferObject);
		mesh_directional_light_shadow_perdrawcall_vertex_blending_storage_buffer_info.buffer = m_global_render_resource->m_storage_buffer.m_global_upload_ringbuffer;

		ASSERT(mesh_directional_light_shadow_perdrawcall_vertex_blending_storage_buffer_info.range < m_global_render_resource->m_storage_buffer.m_max_storage_buffer_range);
		...
		RHIWriteDescriptorSet& mesh_directional_light_shadow_perdrawcall_vertex_blending_buffer_write_info = descriptor_writes[2];
		mesh_directional_light_shadow_perdrawcall_vertex_blending_buffer_write_info.sType = RHI_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
		mesh_directional_light_shadow_perdrawcall_vertex_blending_buffer_write_info.pNext = nullptr;
		mesh_directional_light_shadow_perdrawcall_vertex_blending_buffer_write_info.dstSet = descriptor_set_to_write;
		mesh_directional_light_shadow_perdrawcall_vertex_blending_buffer_write_info.dstBinding = 2;
		mesh_directional_light_shadow_perdrawcall_vertex_blending_buffer_write_info.dstArrayElement = 0;
		mesh_directional_light_shadow_perdrawcall_vertex_blending_buffer_write_info.descriptorType = RHI_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC;
		mesh_directional_light_shadow_perdrawcall_vertex_blending_buffer_write_info.descriptorCount = 1;
		mesh_directional_light_shadow_perdrawcall_vertex_blending_buffer_write_info.pBufferInfo = &mesh_directional_light_shadow_perdrawcall_vertex_blending_storage_buffer_info;
			...
			for (auto& [material, mesh_instanced] : directional_light_mesh_drawcall_batch) {
				...
				for (auto& [mesh, mesh_nodes] : mesh_instanced) {
					...
					if (total_instance_count > 0) {
						...
						uint32_t drawcall_max_instance_count = sizeof(MeshDirectionalLightShadowPerdrawcallStorageBufferObject::mesh_instances) / sizeof(MeshDirectionalLightShadowPerdrawcallStorageBufferObject::mesh_instances[0]);
						...
						for (uint32_t drawcall_index = 0; drawcall_index < drawcall_count; ++drawcall_index) {
							...
							MeshDirectionalLightShadowPerdrawcallStorageBufferObject& perdrawcall_storage_buffer_object = (*reinterpret_cast<MeshDirectionalLightShadowPerdrawcallStorageBufferObject*>(
							...
							//perdrawcall vertex blending storage buffer
							...
							if (enable_vertex_blending) {
								...
								MeshDirectionalLightShadowPerDrawcallVertexBlendingStorageBufferObject& perdrawcall_vertex_blending_storage_buffer_object = (*reinterpret_cast<MeshDirectionalLightShadowPerDrawcallVertexBlendingStorageBufferObject*>(
									reinterpret_cast<uintptr_t>(m_global_render_resource->m_storage_buffer.m_global_upload_ringbuffer_memory_pointer) + per_drawcall_vertex_blending_dynamic_offset));
								for (uint32_t i = 0; i < current_instance_count; ++i) {
									if (mesh_nodes[drawcall_max_instance_count * drawcall_index + i].joint_matrices) {
