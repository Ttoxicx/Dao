...
namespace Dao {
	void FXAAPass::initialize(const RenderPassInitInfo* init_info) {
		...
		const FXAAPassInitInfo* info = static_cast<const FXAAPassInitInfo*>(init_info);
	...
	void FXAAPass::setupDescriptorSetLayout() {
		...
		if (m_rhi->createDescriptorSetLayout(&post_process_global_layout_create_info, m_descriptor_infos[0].layout) != RHI_SUCCESS) {
			LOG_FATAL("falied to create post process global layout");
		}
	}
	...
	void FXAAPass::setupPipelines() {
		...
		RHIPipelineShaderStageCreateInfo shader_stages[] = { vert_pipeline_shader_create_info,frag_pipeline_shader_create_info };
		...
		pipeline_info.subpass = main_camera_subpass_fxaa;
	...
	void FXAAPass::updateAfterFramebufferRecreate(RHIImageView* input_attachment) {
		RHIDescriptorImageInfo post_process_perframe_input_attachment_info = {};
		post_process_perframe_input_attachment_info.sampler = m_rhi->getOrCreateDefaultSampler(Default_Sampler_Linear);
		...
		post_process_descriptor_input_attachment_write_info.pImageInfo = &post_process_perframe_input_attachment_info;
	...
	void FXAAPass::draw() {
		...
		RHIRect2D scissor = {
			static_cast<int32_t>(m_rhi->getSwapChainInfo().viewport->x),
			static_cast<int32_t>(m_rhi->getSwapChainInfo().viewport->y),
			static_cast<uint32_t>(m_rhi->getSwapChainInfo().viewport->width),
			static_cast<uint32_t>(m_rhi->getSwapChainInfo().viewport->height)
		};
