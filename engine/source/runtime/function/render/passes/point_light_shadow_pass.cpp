#include "runtime/function/render/passes/point_light_shadow_pass.h"
...
namespace Dao {
	...
	void PointLightShadowPass::preparePassData(std::shared_ptr<RenderResourceBase> render_resource) {
		...
		if (vulkan_resource) {
			_mesh_point_light_shadow_perframe_storage_buffer_object = vulkan_resource->m_mesh_point_light_shadow_perframe_storage_buffer_object;
		}
	}
	...
	void PointLightShadowPass::setupAttachment() {
		//color and depth
		...
		m_rhi->createImage(
			...
			m_framebuffer.attachments[0].memory,
			0, 2 * s_max_point_light_count, 1
		);
		...
		m_rhi->createImage(
			...
			m_framebuffer.attachments[1].memory,
			0, 2 * s_max_point_light_count, 1
		);
		m_rhi->createImageView(
			m_framebuffer.attachments[1].image,
	...
	void PointLightShadowPass::setupRenderPass() {
		...
		RHIAttachmentDescription& point_light_shadow_stencil_depth_attachment_description = attachments[1];
		point_light_shadow_stencil_depth_attachment_description.format = m_framebuffer.attachments[1].format;
		point_light_shadow_stencil_depth_attachment_description.samples = RHI_SAMPLE_COUNT_1_BIT;
		point_light_shadow_stencil_depth_attachment_description.loadOp = RHI_ATTACHMENT_LOAD_OP_CLEAR;
		point_light_shadow_stencil_depth_attachment_description.storeOp = RHI_ATTACHMENT_STORE_OP_DONT_CARE;
		point_light_shadow_stencil_depth_attachment_description.stencilLoadOp = RHI_ATTACHMENT_LOAD_OP_DONT_CARE;
		point_light_shadow_stencil_depth_attachment_description.stencilStoreOp = RHI_ATTACHMENT_STORE_OP_DONT_CARE;
		point_light_shadow_stencil_depth_attachment_description.initialLayout = RHI_IMAGE_LAYOUT_UNDEFINED;
		point_light_shadow_stencil_depth_attachment_description.finalLayout = RHI_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
		...
		shadow_pass_depth_stencil_attachment_reference.layout = RHI_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
		...
		shadow_pass.pDepthStencilAttachment = &shadow_pass_depth_stencil_attachment_reference;
		...
		lighting_pass_dependency.srcAccessMask = RHI_ACCESS_COLOR_ATTACHMENT_WRITE_BIT;
		lighting_pass_dependency.dstAccessMask = 0;
		lighting_pass_dependency.dependencyFlags = 0;
		...
		if (m_rhi->createRenderPass(&renderpass_create_info, m_framebuffer.render_pass) != RHI_SUCCESS) {
			LOG_FATAL("failed to create point light shadow render pass");
		}
	}
	...
	void PointLightShadowPass::setupFramebuffer() {
		...
		if (m_rhi->createFramebuffer(&framebuffer_create_info, m_framebuffer.framebuffer) != RHI_SUCCESS) {
			LOG_FATAL("failed to create light shadow framebuffer");
		}
	}
	...
	void PointLightShadowPass::setupDescriptorSetLayout() {
		...
		RHIDescriptorSetLayoutBinding& mesh_point_light_shadow_global_layout_perframe_storage_buffer_binding = mesh_point_light_shadow_global_layout_bindings[0];
		mesh_point_light_shadow_global_layout_perframe_storage_buffer_binding.binding = 0;
		mesh_point_light_shadow_global_layout_perframe_storage_buffer_binding.descriptorType = RHI_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC;
		mesh_point_light_shadow_global_layout_perframe_storage_buffer_binding.descriptorCount = 1;
		mesh_point_light_shadow_global_layout_perframe_storage_buffer_binding.stageFlags = RHI_SHADER_STAGE_GEOMETRY_BIT | RHI_SHADER_STAGE_FRAGMENT_BIT;

		RHIDescriptorSetLayoutBinding& mesh_point_light_shadow_global_layout_perdrawcall_storage_buffer_binding = mesh_point_light_shadow_global_layout_bindings[1];
		mesh_point_light_shadow_global_layout_perdrawcall_storage_buffer_binding.binding = 1;
		mesh_point_light_shadow_global_layout_perdrawcall_storage_buffer_binding.descriptorType = RHI_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC;
		...
		RHIDescriptorSetLayoutBinding& mesh_point_light_shadow_global_layout_perdrawcall_vertex_blending_storage_buffer_binding = mesh_point_light_shadow_global_layout_bindings[2];
		mesh_point_light_shadow_global_layout_perdrawcall_vertex_blending_storage_buffer_binding.binding = 2;
		mesh_point_light_shadow_global_layout_perdrawcall_vertex_blending_storage_buffer_binding.descriptorType = RHI_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC;
		mesh_point_light_shadow_global_layout_perdrawcall_vertex_blending_storage_buffer_binding.descriptorCount = 1;
		mesh_point_light_shadow_global_layout_perdrawcall_vertex_blending_storage_buffer_binding.stageFlags = RHI_SHADER_STAGE_VERTEX_BIT;
		...
		mesh_point_light_shadow_global_layout_create_info.bindingCount = sizeof(mesh_point_light_shadow_global_layout_bindings) / sizeof(mesh_point_light_shadow_global_layout_bindings[0]);
		mesh_point_light_shadow_global_layout_create_info.pBindings = mesh_point_light_shadow_global_layout_bindings;

		if (m_rhi->createDescriptorSetLayout(&mesh_point_light_shadow_global_layout_create_info, m_descriptor_infos[0].layout) != RHI_SUCCESS) {
			LOG_FATAL("failed to create mesh point light shadow global layout");
		}
	}
	...
	void PointLightShadowPass::setupPipelines() {
		...
		if (m_rhi->createPipelineLayout(&pipeline_layout_create_info, m_render_pipelines[0].layout) != RHI_SUCCESS) {
			LOG_FATAL("failed to create mesh point light shadow pipeline layout");
		}
		...
		RHIPipelineShaderStageCreateInfo shader_stages[] = { vert_pipeline_shader_stage_create_info,geom_pipeline_shader_stage_create_info,frag_pipeline_shader_stage_create_info };
		...
		if (m_rhi->createGraphicsPipelines(RHI_NULL_HANDLE, 1, &pipeline_info, m_render_pipelines[0].pipeline) != RHI_SUCCESS) {
			LOG_FATAL("failed to create mesh point light shadow graphics pipeline");
		}
	...
	void PointLightShadowPass::setupDescriptorSet() {
		...
		if (m_rhi->allocateDescriptorSets(&mesh_point_light_shadow_global_descriptor_set_alloc_info, m_descriptor_infos[0].descriptor_set) != RHI_SUCCESS) {
			LOG_FATAL("failed to allocate mesh point light shadow global descriptor set");
		}
		...
		mesh_point_light_shadow_perframe_storage_buffer_info.range = sizeof(MeshPointLightShadowPerframeStorageBufferObject);
		mesh_point_light_shadow_perframe_storage_buffer_info.buffer = m_global_render_resource->m_storage_buffer.m_global_upload_ringbuffer;

		ASSERT(mesh_point_light_shadow_perframe_storage_buffer_info.range < m_global_render_resource->m_storage_buffer.m_max_storage_buffer_range);
		...
		mesh_point_light_shadow_perdrawcall_storage_buffer_info.range = sizeof(MeshPointLightShadowPerdrawcallStorageBufferObject);
		mesh_point_light_shadow_perdrawcall_storage_buffer_info.buffer = m_global_render_resource->m_storage_buffer.m_global_upload_ringbuffer;

		ASSERT(mesh_point_light_shadow_perdrawcall_storage_buffer_info.range < m_global_render_resource->m_storage_buffer.m_max_storage_buffer_range);

		RHIDescriptorBufferInfo mesh_point_light_shadow_perdrawcall_vertex_blending_storage_buffer_info{};
		mesh_point_light_shadow_perdrawcall_vertex_blending_storage_buffer_info.offset = 0;
		mesh_point_light_shadow_perdrawcall_vertex_blending_storage_buffer_info.range = sizeof(MeshPointLightShadowPerdrawcallVertexBlendingStorageBufferObject);
		mesh_point_light_shadow_perdrawcall_vertex_blending_storage_buffer_info.buffer = m_global_render_resource->m_storage_buffer.m_global_upload_ringbuffer;

		ASSERT(mesh_point_light_shadow_perdrawcall_vertex_blending_storage_buffer_info.range < m_global_render_resource->m_storage_buffer.m_max_storage_buffer_range);

		RHIDescriptorSet* descriptor_set_to_write = m_descriptor_infos[0].descriptor_set;

		RHIWriteDescriptorSet descriptor_writes[3];

		RHIWriteDescriptorSet& mesh_point_perframe_storage_buffer_write_info = descriptor_writes[0];
		mesh_point_perframe_storage_buffer_write_info.sType = RHI_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
		mesh_point_perframe_storage_buffer_write_info.pNext = nullptr;
		mesh_point_perframe_storage_buffer_write_info.dstSet = descriptor_set_to_write;
		mesh_point_perframe_storage_buffer_write_info.dstBinding = 0;
		mesh_point_perframe_storage_buffer_write_info.dstArrayElement = 0;
		mesh_point_perframe_storage_buffer_write_info.descriptorType = RHI_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC;
		mesh_point_perframe_storage_buffer_write_info.descriptorCount = 1;
		mesh_point_perframe_storage_buffer_write_info.pBufferInfo = &mesh_point_light_shadow_perframe_storage_buffer_info;

		RHIWriteDescriptorSet& mesh_point_perdrawcall_storage_buffer_write_info = descriptor_writes[1];
		mesh_point_perdrawcall_storage_buffer_write_info.sType = RHI_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
		mesh_point_perdrawcall_storage_buffer_write_info.pNext = nullptr;
		mesh_point_perdrawcall_storage_buffer_write_info.dstSet = descriptor_set_to_write;
		mesh_point_perdrawcall_storage_buffer_write_info.dstBinding = 1;
		mesh_point_perdrawcall_storage_buffer_write_info.dstArrayElement = 0;
		mesh_point_perdrawcall_storage_buffer_write_info.descriptorType = RHI_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC;
		mesh_point_perdrawcall_storage_buffer_write_info.descriptorCount = 1;
		mesh_point_perdrawcall_storage_buffer_write_info.pBufferInfo = &mesh_point_light_shadow_perdrawcall_storage_buffer_info;

		RHIWriteDescriptorSet& mesh_point_perdrawcall_vertex_blending_storage_buffer_write_info = descriptor_writes[2];
		mesh_point_perdrawcall_vertex_blending_storage_buffer_write_info.sType = RHI_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
		mesh_point_perdrawcall_vertex_blending_storage_buffer_write_info.pNext = nullptr;
		mesh_point_perdrawcall_vertex_blending_storage_buffer_write_info.dstSet = descriptor_set_to_write;
		mesh_point_perdrawcall_vertex_blending_storage_buffer_write_info.dstBinding = 2;
		mesh_point_perdrawcall_vertex_blending_storage_buffer_write_info.dstArrayElement = 0;
		mesh_point_perdrawcall_vertex_blending_storage_buffer_write_info.descriptorType = RHI_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC;
		mesh_point_perdrawcall_vertex_blending_storage_buffer_write_info.descriptorCount = 1;
		mesh_point_perdrawcall_vertex_blending_storage_buffer_write_info.pBufferInfo = &mesh_point_light_shadow_perdrawcall_vertex_blending_storage_buffer_info;
	...
	void PointLightShadowPass::drawModel() {
		...
		renderpass_begin_info.renderArea.extent = { s_point_light_shadow_map_dimension,s_point_light_shadow_map_dimension };
			...
			for (auto& pair1 : point_lights_mesh_drawcall_batch) {
				...
				for (auto& pair2 : mesh_instanced) {
					...
					if (total_instance_count > 0) {
						...
						uint32_t drawcall_max_instance_count = (sizeof(MeshPointLightShadowPerdrawcallStorageBufferObject::mesh_instances) / sizeof(MeshPointLightShadowPerdrawcallStorageBufferObject::mesh_instances[0]));
						...
						for (uint32_t drawcall_index = 0; drawcall_index < drawcall_count; ++drawcall_index) {
							...
							m_global_render_resource->m_storage_buffer.m_global_upload_ringbuffers_end[m_rhi->getCurrentFrameIndex()] = perdrawcall_dynamic_offset + sizeof(MeshPointLightShadowPerdrawcallStorageBufferObject);
							...
							MeshPointLightShadowPerdrawcallStorageBufferObject& perdrawcall_storage_buffer_object = (*reinterpret_cast<MeshPointLightShadowPerdrawcallStorageBufferObject*>(
								reinterpret_cast<uintptr_t>(m_global_render_resource->m_storage_buffer.m_global_upload_ringbuffer_memory_pointer) + perdrawcall_dynamic_offset));
							...
							if (mesh.enable_vertex_blending) {
								perdrawcall_vertex_blending_dynamic_offset = roundUp(
									m_global_render_resource->m_storage_buffer.m_global_upload_ringbuffers_end[m_rhi->getCurrentFrameIndex()],
									m_global_render_resource->m_storage_buffer.m_min_storage_buffer_offset_alignment
								);
								m_global_render_resource->m_storage_buffer.m_global_upload_ringbuffers_end[m_rhi->getCurrentFrameIndex()] = perdrawcall_vertex_blending_dynamic_offset + sizeof(MeshPointLightShadowPerdrawcallVertexBlendingStorageBufferObject);
								...
								MeshPointLightShadowPerdrawcallVertexBlendingStorageBufferObject& perdrawcall_vertex_blending_storage_buffer_object = (*reinterpret_cast<MeshPointLightShadowPerdrawcallVertexBlendingStorageBufferObject*>(
									reinterpret_cast<uintptr_t>(m_global_render_resource->m_storage_buffer.m_global_upload_ringbuffer_memory_pointer) + perdrawcall_vertex_blending_dynamic_offset));
