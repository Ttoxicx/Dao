...
namespace Dao {
	...
	void GameObjectResourceDesc::add(GameObjectDesc& desc) {
		m_game_object_descs.push_back(desc);
	}

	bool GameObjectResourceDesc::isEmpty() const {
		return m_game_object_descs.empty();
	}
	...
	void EmitterTransformRequest::add(ParticleEmitterTransformDesc& desc) {
		m_transform_descs.push_back(desc);
	}

	void EmitterTransformRequest::clear() {
		m_transform_descs.clear();
	}

	unsigned int EmitterTransformRequest::getEmitterCount() const {
		return m_transform_descs.size();
	}
	...
	void RenderSwapData::addDirtyGameObject(GameObjectDesc&& desc) {
		if (m_game_object_resource_desc.has_value()) {
			...
			GameObjectResourceDesc go_desc;
			go_desc.add(desc);
			m_game_object_resource_desc = go_desc;
		}
	}
	...
	void RenderSwapData::addDeleteGameObject(GameObjectDesc&& desc) {
		if (m_game_object_to_delete.has_value()) {
			...
			GameObjectResourceDesc go_desc;
			go_desc.add(desc);
			m_game_object_to_delete = go_desc;
		}
	}
