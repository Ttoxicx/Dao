...
namespace Dao {
	...
	void RenderSystem::initialize(RenderSystemInitInfo init_info) {
		...
		m_render_resource->uploadGloabalRenderResource(m_rhi, level_resource_desc);
		//setup render camera
		...
		pipeline_init_info.m_enable_fxaa = global_rendering_res.m_enable_fxaa;
		pipeline_init_info.m_render_resource = m_render_resource;
		m_render_pipeline = std::make_shared<RenderPipeline>();
		m_render_pipeline->m_rhi = m_rhi;
		m_render_pipeline->initialize(pipeline_init_info);
		//descriptor set layout in main camera pass will be used when upload resource
		std::static_pointer_cast<RenderResource>(m_render_resource)->m_mesh_descriptor_set_layout = &static_cast<RenderPass*>(m_render_pipeline->m_main_camera_pass.get())->m_descriptor_infos[MainCameraPass::LayoutType::layout_type_per_mesh].layout;
		std::static_pointer_cast<RenderResource>(m_render_resource)->m_material_descriptor_set_layout = &static_cast<RenderPass*>(m_render_pipeline->m_main_camera_pass.get())->m_descriptor_infos[MainCameraPass::LayoutType::layout_type_mesh_per_material].layout;
	}

	void RenderSystem::tick(float delta_time) {
		//process swap date between logic and render contexts
		processSwapData();
		//prepare render command context;
		m_rhi->prepareContext();
		//update perframe buffer
		m_render_resource->updatePerFrameBuffer(m_render_scene, m_render_camera);
		//update perframe visible objects
		...
		if (m_render_pipeline_type == RENDER_PIPELINE_TYPE::FORWARD_PIPELINE) {
			...
			LOG_ERROR("unsupported render pipeline type");
		}
	}
	...
	void RenderSystem::updateEngineContentViewport(float offset_x, float offset_y, float width, float height) {
		auto vk_rhi = std::static_pointer_cast<VulkanRHI>(m_rhi);
		vk_rhi->m_viewport.x = offset_x;
		vk_rhi->m_viewport.y = offset_y;
		vk_rhi->m_viewport.width = width;
		vk_rhi->m_viewport.height = height;
		vk_rhi->m_viewport.minDepth = 0.0f;
		vk_rhi->m_viewport.maxDepth = 1.0f;
		m_render_camera->setAspect(width / height);
	}

	EngineContentViewport RenderSystem::getEngineContentViewport() const {
		auto vk_rhi = std::static_pointer_cast<VulkanRHI>(m_rhi);
		float x = vk_rhi->m_viewport.x;
		float y = vk_rhi->m_viewport.y;
		float width = vk_rhi->m_viewport.width;
		float height = vk_rhi->m_viewport.height;
	...
	void RenderSystem::processSwapData() {
		...
		//TODO(update gloabal resources is needed)
		//update level resource 
		if (swap_data.m_level_resource_desc.has_value()) {
			m_render_resource->uploadGloabalRenderResource(m_rhi, *swap_data.m_level_resource_desc);
			//reset level resource swap data to a clean state
			m_swap_context.resetLevelResourceSwapData();
		}
		//update geme object if needed
			...
			while (!swap_data.m_game_object_resource_desc->isEmpty()) {
				...
				for (size_t part_index = 0; part_index < gobject.getObjectParts().size(); ++part_index) {
					...
					if (game_object_part.m_material_desc.m_with_texture) {
						material_source = {
							game_object_part.m_material_desc.m_base_color_texture_file,
					...
					bool is_material_loaded = m_render_scene->getMaterialAssetIdAllocator().hasElement(material_source);
					RenderMaterialData material_data;
					if (!is_material_loaded) {
						material_data = m_render_resource->loadMaterialData(material_source);
					}
					render_entity.m_material_asset_id = m_render_scene->getMaterialAssetIdAllocator().allocGuid(material_source);
				...
				//after finish processing,pop this game object
				swap_data.m_game_object_resource_desc->pop();
			}
			//reset game object swap data to clean state
		...
		//process particle submit request
		...
		//process emitter transform request
		if (swap_data.m_emitter_transform_request.has_value()) {
			std::static_pointer_cast<ParticlePass>(m_render_pipeline->m_particle_pass)->setTransformIndices(swap_data.m_emitter_transform_request->m_transform_descs);
