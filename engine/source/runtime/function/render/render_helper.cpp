...
namespace Dao {
    ...
    bool TiledFrustumIntersectBox(ClusterFrustum const& f, BoundingBox const& b)
    {
        ...
        // Center of the box.
        Vector4 box_center((b.max_bound.x + b.min_bound.x) * 0.5, (b.max_bound.y + b.min_bound.y) * 0.5, (b.max_bound.z + b.min_bound.z) * 0.5, 1.0);

        // Distance from the center to each side.
        // half extent //more exactly
        Vector3 box_extents((b.max_bound.x - b.min_bound.x) * 0.5, (b.max_bound.y - b.min_bound.y) * 0.5, (b.max_bound.z - b.min_bound.z) * 0.5);

        // plane_right
        {
            float signed_distance_from_plane_right = f.m_plane_right.dotProduct(box_center);
            float radius_project_plane_right = Vector3(fabs(f.m_plane_right.x), fabs(f.m_plane_right.y), fabs(f.m_plane_right.z)).dotProduct(box_extents);
        ...
        // plane_left
        {
            ...
            float radius_project_plane_left = Vector3(fabs(f.m_plane_left.x), fabs(f.m_plane_left.y), fabs(f.m_plane_left.z)).dotProduct(box_extents);
        ...
        // plane_top
        {
            ...
            float radius_project_plane_top = Vector3(fabs(f.m_plane_top.x), fabs(f.m_plane_top.y), fabs(f.m_plane_top.z)).dotProduct(box_extents);
        ...
        // plane_bottom
        {
            ...
            float radius_project_plane_bottom = Vector3(fabs(f.m_plane_bottom.x), fabs(f.m_plane_bottom.y), fabs(f.m_plane_bottom.z)).dotProduct(box_extents);
        ...
        // plane_near
        {
            ...
            float radius_project_plane_near = Vector3(fabs(f.m_plane_near.x), fabs(f.m_plane_near.y), fabs(f.m_plane_near.z)).dotProduct(box_extents);
        ...
        // plane_far
        {
            ...
            float radius_project_plane_far = Vector3(fabs(f.m_plane_far.x), fabs(f.m_plane_far.y), fabs(f.m_plane_far.z)).dotProduct(box_extents);
    ...
    BoundingBox BoundingBoxTransform(BoundingBox const& b, Matrix4x4 const& m)
    {
        ...
        // Center of the box.
        Vector3 center((b.max_bound.x + b.min_bound.x) * 0.5, (b.max_bound.y + b.min_bound.y) * 0.5, (b.max_bound.z + b.min_bound.z) * 0.5);

        // Distance from the center to each side.
        // half extent //more exactly
        Vector3 extents((b.max_bound.x - b.min_bound.x) * 0.5, (b.max_bound.y - b.min_bound.y) * 0.5, (b.max_bound.z - b.min_bound.z) * 0.5);
        ...
        for (size_t i = 0; i < CORNER_COUNT; ++i)
        {
            ...
            Vector3 corner = Vector3(corner_with_w.x / corner_with_w.w, corner_with_w.y / corner_with_w.w, corner_with_w.z / corner_with_w.w);
    ...
    Matrix4x4 CalculateDirectionalLightCamera(RenderScene& scene, RenderCamera& camera)
    {
        ...
        BoundingBox scene_bounding_box;
        {
            ...
            for (const RenderEntity& entity : scene.m_render_entities)
            {
                ...
                BoundingBox mesh_bounding_box_world = BoundingBoxTransform(mesh_asset_bounding_box, entity.m_model_matrix);
        ...
        {
            Vector3 box_center(
                (frustum_bounding_box.max_bound.x + frustum_bounding_box.min_bound.x) * 0.5,
