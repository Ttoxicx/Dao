...
namespace Dao {
	void RenderPipeline::initialize(RenderPipelineInitInfo init_info) {
		...
		pass_common_info.m_rhi = m_rhi;
		pass_common_info.m_render_resource = init_info.m_render_resource;

		m_point_light_shadow_pass->setCommonInfo(pass_common_info);
		m_directional_light_shadow_pass->setCommonInfo(pass_common_info);
		...
		m_directional_light_shadow_pass->initialize(nullptr);
		...
		m_main_camera_pass->initialize(&main_camera_pass_init_info);

		particle_pass->setupParticlePass();

		std::vector<RHIDescriptorSetLayout*> descriptor_layouts = std::static_pointer_cast<RenderPass>(m_main_camera_pass)->getDescriptorSetLayouts();
		std::static_pointer_cast<PointLightShadowPass>(m_point_light_shadow_pass)->setPerMeshLayout(descriptor_layouts[MainCameraPass::LayoutType::layout_type_per_mesh]);
		std::static_pointer_cast<DirectionalLightShadowPass>(m_directional_light_shadow_pass)->setPerMeshLayout(descriptor_layouts[MainCameraPass::LayoutType::layout_type_per_mesh]);
		...
		tone_mapping_init_info.render_pass = main_camera_pass->getRenderPass();
		tone_mapping_init_info.input_attachment = main_camera_pass->getFrameBufferImageViews()[main_camera_pass_backup_buffer_odd];
		m_tone_mapping_pass->initialize(&tone_mapping_init_info);

		ColorGradingPassInitInfo color_grading_init_info;
		color_grading_init_info.render_pass = main_camera_pass->getRenderPass();
		color_grading_init_info.input_attachment = main_camera_pass->getFrameBufferImageViews()[main_camera_pass_backup_buffer_even];
		m_color_grading_pass->initialize(&color_grading_init_info);

		UIPassInitInfo ui_init_info;
		ui_init_info.render_pass = main_camera_pass->getRenderPass();
		m_ui_pass->initialize(&ui_init_info);

		CombineUIPassInitInfo combine_ui_init_info;
		combine_ui_init_info.render_pass = main_camera_pass->getRenderPass();
		combine_ui_init_info.scene_input_attachment = main_camera_pass->getFrameBufferImageViews()[main_camera_pass_backup_buffer_odd];
		combine_ui_init_info.ui_input_attachment = main_camera_pass->getFrameBufferImageViews()[main_camera_pass_backup_buffer_even];
		m_combine_ui_pass->initialize(&combine_ui_init_info);

		PickPassInitInfo pick_init_info;
		pick_init_info.per_mesh_layout = descriptor_layouts[MainCameraPass::LayoutType::layout_type_per_mesh];
	...
	void RenderPipeline::forwardRender(std::shared_ptr<RHI> rhi, std::shared_ptr<RenderResourceBase> render_resource) {
		auto vk_rhi = static_cast<VulkanRHI*>(rhi.get());
		auto vk_resource = static_cast<RenderResource*>(render_resource.get());
		vk_resource->resetRingBufferOffset(vk_rhi->m_current_frame_index);
		vk_rhi->waitForFences();
		vk_rhi->resetCommandPool();
		bool recreate_swapchain = vk_rhi->prepareBeforePass(std::bind(&RenderPipeline::passUpdateAfterRecreateSwapchain, this));
		...
		vk_rhi->submitRendering(std::bind(&RenderPipeline::passUpdateAfterRecreateSwapchain, this));
