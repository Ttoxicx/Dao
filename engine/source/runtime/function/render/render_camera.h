...
namespace Dao {
	...
	public:
		...
		void setAspect(float acpect);
		void setFOVx(float fovx) { m_fovx = fovx; }

		Vector3 position() const { return m_position; }
		Quaternion rotation() const { return m_rotation; }
		Vector3 forward() const { return m_inv_rotation * Y; }
		Vector3 up() const { return m_inv_rotation * Z; }
	...
	protected:
		float m_aspect{ 0.0f };
		float m_fovx{ Degree(89.0f).valueDegrees() };
		float m_fovy{ 0.0f };
