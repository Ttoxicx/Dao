...
namespace Dao {
    ...
    size_t EditorSceneManager::updateCursorOnAxis(Vector2 cursor_uv, Vector2 game_engine_window_size) {
        ...
        float   camera_fov = _camera->getFovYDeprecated();
        Vector3 camera_forward = _camera->forward();

        Vector3 camera_up = _camera->up();
        Vector3 camera_right = _camera->right();
        Vector3 camera_position = _camera->position();

        if (_selected_gobject_id == k_invalid_gobject_id) {
            return _selected_axis;
        }
        RenderEntity* selected_aixs = getAxisMeshByType(_axis_mode);
        _selected_axis = 3;
        if (_is_show_axis == false) {
            return _selected_axis;
        }
        else {
            ...
            Vector3 world_ray_dir = camera_forward * window_forward + camera_right * (float)game_engine_window_size.x * screen_center_uv.x + camera_up * (float)game_engine_window_size.y * screen_center_uv.y;
            ...
            float plane_view_depth[3] = {
                intersectPlaneRay(plane_normals[0], 0, local_ray_origin_xyz, local_ray_dir),
            ...
            if ((int)_axis_mode == 0 || (int)_axis_mode == 2) {// transition axis & scale axis
            ...
            else if ((int)_axis_mode == 1) { // rotation axis
        ...
        g_editor_global_context.m_render_system->setSelectedAxis(_selected_axis);
    ...
    RenderEntity* EditorSceneManager::getAxisMeshByType(EditorAxisMode axis_mode) {
        ...
        case EditorAxisMode::TranslateMode:
            axis_mesh = &_translation_axis;
            break;
        case EditorAxisMode::RotateMode:
            axis_mesh = &_rotation_axis;
            break;
        case EditorAxisMode::ScaleMode:
            axis_mesh = &_scale_aixs;
            break;
        default:
            break;
        }
    ...
    void EditorSceneManager::drawSelectedEntityAxis() {
        ...
        if (g_is_editor_mode && selected_object != nullptr) {
            ...
            RenderEntity* selected_aixs = getAxisMeshByType(_axis_mode);
            if (_axis_mode == EditorAxisMode::TranslateMode || _axis_mode == EditorAxisMode::RotateMode) {
                selected_aixs->m_model_matrix = axis_model_matrix;
            }
            else if (_axis_mode == EditorAxisMode::ScaleMode) {
    ...
    std::weak_ptr<GObject> EditorSceneManager::getSelectedGObject() const {
        ...
        if (_selected_gobject_id != k_invalid_gobject_id) {
            ...
            if (level != nullptr) {
                selected_object = level->getGObjectByID(_selected_gobject_id);
            }
        }
    ...
    void EditorSceneManager::onGObjectSelected(GObjectID selected_gobject_id) {
        if (selected_gobject_id == _selected_gobject_id) {
            return;
        }

        _selected_gobject_id = selected_gobject_id;

        std::shared_ptr<GObject> selected_gobject = getSelectedGObject().lock();
        if (selected_gobject) {
            const TransformComponent* transform_component = selected_gobject->tryGetComponentConst(TransformComponent);
            _selected_object_matrix = transform_component->getMatrix();
        }

        drawSelectedEntityAxis();

        if (_selected_gobject_id != k_invalid_gobject_id) {
            LOG_INFO("select game object " + std::to_string(_selected_gobject_id));
        }
        else {
    ...
    void EditorSceneManager::onDeleteSelectedGObject() {
        ...
        if (selected_object != nullptr) {
            ...
            current_active_level->deleteGObjectByID(_selected_gobject_id);
    ...
    void EditorSceneManager::moveEntity(
        ...
        Matrix4x4 view_matrix = _camera->getLookAtMatrix();
        Matrix4x4 proj_matrix = _camera->getPersProjMatrix();
        ...
        if (_axis_mode == EditorAxisMode::ScaleMode) {
        ...
        if (_axis_mode == EditorAxisMode::TranslateMode) { // translate
            ...
            _translation_axis.m_model_matrix = axis_model_matrix;
            _rotation_axis.m_model_matrix = axis_model_matrix;
            _scale_aixs.m_model_matrix = axis_model_matrix;

            g_editor_global_context.m_render_system->setVisibleAxis(_translation_axis);
        ...
        else if (_axis_mode == EditorAxisMode::RotateMode) { // rotate
            ...
            if (cursor_on_axis == 0) {
                ...
                if (_camera->forward().dotProduct(Vector3::UNIT_X) < 0)
                {
                ...
                move_radian = (delta_mouse_move_uv * angularVelocity).length();
                if (_camera->forward().dotProduct(Vector3::UNIT_Y) < 0)
                {
                    move_radian = -move_radian;
                }
                ...
                move_radian = (delta_mouse_move_uv * angularVelocity).length();
                if (_camera->forward().dotProduct(Vector3::UNIT_Z) < 0)
                {
                    move_radian = -move_radian;
                }
            ...
            transform_component->setRotation(new_rotation);
            transform_component->setScale(new_scale);
            _scale_aixs.m_model_matrix = new_model_matrix;
        }
        else if (_axis_mode == EditorAxisMode::ScaleMode) { // scale
    ...
    void EditorSceneManager::uploadAxisResource() {
        ...
        // assign some value that won't be used by other game objects
        {
            ...
            _translation_axis.m_instance_id = instance_id_allocator.allocGuid(axis_instance_id);
            _translation_axis.m_mesh_asset_id = mesh_asset_id_allocator.allocGuid(mesh_source_desc);
        }

        {
            ...
            _rotation_axis.m_instance_id = instance_id_allocator.allocGuid(axis_instance_id);
            _rotation_axis.m_mesh_asset_id = mesh_asset_id_allocator.allocGuid(mesh_source_desc);
        }

        {
            ...
            _scale_aixs.m_instance_id = instance_id_allocator.allocGuid(axis_instance_id);
            _scale_aixs.m_mesh_asset_id = mesh_asset_id_allocator.allocGuid(mesh_source_desc);
        }

        g_editor_global_context.m_render_system->createAxis(
            { _translation_axis, _rotation_axis, _scale_aixs },
            { _translation_axis.m_mesh_data, _rotation_axis.m_mesh_data, _scale_aixs.m_mesh_data }
        );
    }
