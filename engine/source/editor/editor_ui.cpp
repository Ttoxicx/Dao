...
namespace Dao {
    ...
    void DrawVecControl(const std::string& label, Vector3& values, float resetValue = 0.0f, float columnWidth = 100.0f);
    void DrawVecControl(const std::string& label, Quaternion& values, float resetValue = 0.0f, float columnWidth = 100.0f);

    EditorUI::EditorUI() {
        const auto& asset_folder = g_runtime_global_context.m_config_manager->getAssetFolder();
        _editor_ui_creator["TreeNodePush"] = [this](const std::string& name, void* value_ptr) -> void {
        ...
        _editor_ui_creator["TreeNodePop"] = [this](const std::string& name, void* value_ptr) -> void {
        ...
        _editor_ui_creator["Transform"] = [this](const std::string& name, void* value_ptr) -> void {
            if (g_editor_node_state_array[g_node_depth].second) {
        ...
        _editor_ui_creator["bool"] = [this](const std::string& name, void* value_ptr)  -> void {
            if (g_node_depth == -1) {
        ...
        _editor_ui_creator["int"] = [this](const std::string& name, void* value_ptr) -> void {
            if (g_node_depth == -1) {
                std::string label = "##" + name;
                ImGui::Text("%s", name.c_str());
                ImGui::SameLine();
        ...
        _editor_ui_creator["float"] = [this](const std::string& name, void* value_ptr) -> void {
            if (g_node_depth == -1) {
                std::string label = "##" + name;
                ImGui::Text("%s", name.c_str());
                ImGui::SameLine();
        ...
        _editor_ui_creator["Vector3"] = [this](const std::string& name, void* value_ptr) -> void {
        ...
        _editor_ui_creator["Quaternion"] = [this](const std::string& name, void* value_ptr) -> void {
        ...
        _editor_ui_creator["std::string"] = [this, &asset_folder](const std::string& name, void* value_ptr) -> void {
            if (g_node_depth == -1) {
                std::string label = "##" + name;
                ImGui::Text("%s", name.c_str());
                ImGui::SameLine();
    ...
    void EditorUI::showEditorUI() {
        showEditorMenu(&_editor_menu_window_open);
        showEditorWorldObjectsWindow(&_asset_window_open);
        showEditorGameWindow(&_game_engine_window_open);
        showEditorFileContentWindow(&_file_content_window_open);
        showEditorDetailWindow(&_detail_window_open);
    }
    ...
    void EditorUI::showEditorMenu(bool* p_open) {
        ...
        if (ImGui::BeginMenuBar()) {
            ...
            if (ImGui::BeginMenu("Window")) {
                ImGui::MenuItem("World Objects", nullptr, &_asset_window_open);
                ImGui::MenuItem("Game", nullptr, &_game_engine_window_open);
                ImGui::MenuItem("File Content", nullptr, &_file_content_window_open);
                ImGui::MenuItem("Detail", nullptr, &_detail_window_open);
                ImGui::EndMenu();
            }
            ImGui::EndMenuBar();
        }
    ...
    void EditorUI::showEditorWorldObjectsWindow(bool* p_open) {
        ...
        const LevelObjectMap& all_gobjects = current_active_level->getAllGObjects();
        ...
        for (size_t index = 0; index < fields_count; index++) {
            ...
            if (field.isArrayType()) {
                ...
                if (Reflection::TypeMeta::newArrayAccessorFromName(field.getFieldTypeName(), array_accessor)) {
                    ...
                    _editor_ui_creator["TreeNodePush"](std::string(field.getFieldName()) + "[" + std::to_string(array_count) + "]", nullptr);
                    auto item_type_meta_item = Reflection::TypeMeta::newMetaFromName(array_accessor.getElementTypeName());
                    auto item_ui_creator_iterator = _editor_ui_creator.find(item_type_meta_item.getTypeName());
                    for (int index = 0; index < array_count; index++) {
                        if (item_ui_creator_iterator == _editor_ui_creator.end()) {
                            _editor_ui_creator["TreeNodePush"]("[" + std::to_string(index) + "]", nullptr);
                            auto object_instance = Reflection::ReflectionInstance(
                                Reflection::TypeMeta::newMetaFromName(item_type_meta_item.getTypeName().c_str()),
                                array_accessor.get(index, field_instance)
                            );
                            createClassUI(object_instance);
                            _editor_ui_creator["TreeNodePop"]("[" + std::to_string(index) + "]", nullptr);
                        }
                        else {
                            if (item_ui_creator_iterator == _editor_ui_creator.end()) {
                                continue;
                            }
                            _editor_ui_creator[item_type_meta_item.getTypeName()]("[" + std::to_string(index) + "]", array_accessor.get(index, field_instance));
                        }
                    }
                    _editor_ui_creator["TreeNodePop"](field.getFieldName(), nullptr);
                }
            }
            auto ui_creator_iterator = _editor_ui_creator.find(field.getFieldTypeName());
            if (ui_creator_iterator == _editor_ui_creator.end()) {
                ...
                if (field.getTypeMeta(field_meta)) {
                    ...
                    _editor_ui_creator["TreeNodePush"](field_meta.getTypeName(), nullptr);
                    createClassUI(child_instance);
                    _editor_ui_creator["TreeNodePop"](field_meta.getTypeName(), nullptr);
                }
                else {
                    if (ui_creator_iterator == _editor_ui_creator.end()) {
                        continue;
                    }
                    _editor_ui_creator[field.getFieldTypeName()](field.getFieldName(), field.get(instance.m_instance));
                }
            }
            else {
                _editor_ui_creator[field.getFieldTypeName()](field.getFieldName(), field.get(instance.m_instance));
    ...
    void EditorUI::showEditorDetailWindow(bool* p_open) {
        ...
        for (auto component_ptr : selected_object_components) {
            _editor_ui_creator["TreeNodePush"]((component_ptr.getTypeName()).c_str(), nullptr);
            auto object_instance = Reflection::ReflectionInstance(
                Reflection::TypeMeta::newMetaFromName(component_ptr.getTypeName().c_str()),
                component_ptr.operator->()
            );
            createClassUI(object_instance);
            _editor_ui_creator["TreeNodePop"]((component_ptr.getTypeName()).c_str(), nullptr);
        }
        ImGui::End();
    }
    ...
    void EditorUI::showEditorFileContentWindow(bool* p_open) {
        ...
        if (ImGui::BeginTable("File Content", 2, flags)) {
            ...
            if (current_time - _last_file_tree_update > std::chrono::seconds(1)) {
                _editor_file_service.buildEngineFileTree();
                _last_file_tree_update = current_time;
            }
            _last_file_tree_update = current_time;

            EditorFileNode* editor_root_node = _editor_file_service.getEditorRootNode();
    ...
    void EditorUI::showEditorGameWindow(bool* p_open) {
        ...
        if (ImGui::BeginMenuBar()) {
            ...
            if (g_is_editor_mode) {
                ...
                if (ImGui::Button("Game Mode")) {
                    ...
                    g_editor_global_context.m_scene_manager->drawSelectedEntityAxis();
                    g_runtime_global_context.m_input_system->resetInputCommand();
        ...
        if (!g_is_editor_mode) {
            ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f), "Press Left-Alt key to show cursor!");
        }
    ...
    void EditorUI::onFileContentItemClicked(EditorFileNode* node) {
        ...
        const unsigned int new_object_index = ++_new_object_index_map[node->m_file_name];

        ObjectInstanceRes new_object_instance_res;
        new_object_instance_res.m_name = "New_" + Path::getFileNameWithoutExtension(node->m_file_name) + "_" + std::to_string(new_object_index);
    ...
    void EditorUI::setUIColorStyle() {
        ...
        colors[ImGuiCol_TitleBgCollapsed] = ImVec4(0.13f, 0.14f, 0.15f, 0.51f);
        ...
        colors[ImGuiCol_Header] = ImVec4(0.20f, 0.21f, 0.22f, 0.55f);
        ...
        colors[ImGuiCol_TabUnfocused] = ImVec4(0.20f, 0.21f, 0.22f, 1.00f);
        colors[ImGuiCol_TabUnfocusedActive] = ImVec4(0.20f, 0.21f, 0.22f, 1.00f);
        colors[ImGuiCol_DockingPreview] = ImVec4(0.26f, 0.59f, 0.98f, 0.70f);
        colors[ImGuiCol_DockingEmptyBg] = ImVec4(0.20f, 0.21f, 0.22f, 1.00f);
